#pragma once

#include <cinttypes>
#include <array>

#include <BaseBlockCipher.hpp>

namespace crypto {
constexpr size_t RIJNDAEL_BLOCK_SIZE = 16;
constexpr size_t RIJNDAEL_KEY_SIZE = 32;
constexpr uint_fast32_t NUMBER_OF_ROUNDS = 14;
constexpr uint_fast32_t NUMBER_OF_COLUMNS = 4; // Number of columns (32-bit words) comprising the State.
constexpr uint_fast32_t NUMBER_OF_STATE_ROWS = 4;
constexpr uint_fast32_t NUMBER_OF_KEY = 8;     // Number of 32-bit words comprising the Cipher BaseKey.
constexpr uint_fast32_t AMOUNT_OF_ROUND_KEYS =
	NUMBER_OF_COLUMNS * (NUMBER_OF_ROUNDS + 1);     // Number of 32-bit words comprising the Cipher BaseKey.

// SBox and InvSBox helps to perform operations in GF(2 ^ 8) finite field.
constexpr Byte SBox[] = {};

constexpr Byte InvSBox[] = {
	
};

//
constexpr std::array<std::array<Byte, NUMBER_OF_COLUMNS>, 11> Rcon = {
	{
		{0x00, 0x00, 0x00, 0x00},
		{0x01, 0x00, 0x00, 0x00},
		{0x02, 0x00, 0x00, 0x00},
		{0x04, 0x00, 0x00, 0x00},
		{0x08, 0x00, 0x00, 0x00},
		{0x10, 0x00, 0x00, 0x00},
		{0x20, 0x00, 0x00, 0x00},
		{0x40, 0x00, 0x00, 0x00},
		{0x80, 0x00, 0x00, 0x00},
		{0x1b, 0x00, 0x00, 0x00},
		{0x36, 0x00, 0x00, 0x00}
	},
};
} // namespace crypto